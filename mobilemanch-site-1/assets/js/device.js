document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const specCount = Object.values(specs).reduce((count, section) => count + Object.keys(section || {}).length, 0);
        const dataNote = specCount >= 10
            ? `${specCount} specifications available in the MobileManch catalog.`
            : `${specCount} specifications currently available. More public specifications will be added by the automatic catalog sync.`;
        const highlights = [
